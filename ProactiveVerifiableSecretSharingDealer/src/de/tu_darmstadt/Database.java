...
public class Database {
        ...
        try{
            ...
            if (!new File("storedFiles.db").isFile()) {
                ...
                */
                ...
                for (int i = 1; i <= 10; i++) {
                    ShareHolder shareHolder = new ShareHolder("Server" + i, "localhost", 8001 + (i - 1) * 10);
    ...
    public static BigInteger lookupXvalueForShareHolderAndShare(ShareHolder shareHolder, StoredFile storedFile) throws SQLException {
        QueryBuilder<ManyToMany, String> manyToManyQb = manyToManyDao.queryBuilder();
        manyToManyQb.selectColumns(ManyToMany.X_VALUE);
        manyToManyQb.where().eq(ManyToMany.STORED_FILE_ID_FIELD_NAME, storedFile.getName()).and().eq(ManyToMany.SHAREHOLDER_ID_FIELD_NAME, shareHolder.getName());
